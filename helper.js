...
export const evaluatorHelper = (expr, local, global, params, math) => {
    ...
    if ('num' in expr) math.push(expr.num);
        ...
        if (expr.name in params || expr.name in global)
            math.push(params[expr.name] || global[expr.name] || 0);
    ...
    else if ('call' in expr) math.push(evaluate(expr, local, global, params)); // RECURSION
